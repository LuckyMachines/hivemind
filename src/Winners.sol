...
contract Winners is Hub {
    ...
    function _railcarDidEnter(uint256 railcarID) internal override {
        super._railcarDidEnter(railcarID);
        ...
        returns (uint256 payoutAmount)
    {
        ...
        if (totalPlayers > 3) {
            if (tier1Payouts == 4) {
                ...
                payoutAmounts[0] = (prizePool * 10) / 32;
                ...
                payoutAmounts[2] = prizePool / 8;
            } else if (tier2Payouts == 1 && tier3Payouts == 2) {
                payoutAmounts[0] = (prizePool * 10) / 14;
                payoutAmounts[1] = prizePool / 6;
                payoutAmounts[2] = prizePool / 14;
            } else {
                payoutAmounts[0] = (prizePool * 10) / 14;
                payoutAmounts[1] = prizePool / 7;
                payoutAmounts[2] = prizePool / 10;
            }
            payoutAmounts[3] =
                prizePool -
            ...
            if (tier1Payouts == 3) {
                ...
                payoutAmounts[0] = (prizePool * 10) / 14;
                payoutAmounts[1] = (prizePool * 10) / 67;
            } else {
                payoutAmounts[0] = (prizePool * 10) / 14;
