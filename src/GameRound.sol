...
contract GameRound is Hub, VRFConsumerBaseV2Plus {
    ...
    ) Hub(hubRegistryAddress, hubAdmin) VRFConsumerBaseV2Plus(_vrfCoordinator) {
    ...
    function exitPlayersToNextRound(uint256 railcarID) internal {
        _sendRailcarToHub(railcarID, nextRoundHub);
    }
    ...
    function playerIsInHub(uint256 gameID, address playerAddress)
        ...
        returns (bool)
    {
        return RAILCAR.isMember(railcar[gameID], playerAddress);
    }
